...
n-1$ denote the segments with of the coalescent with $i$ lineages. For each $T_i$ \ty{epos}
...
\item A classical problem when estimating model parameters is
  ...
  with respect to the population from which the sample was drawn.  To
  avoid over-fitting in our analysis, we now switch on cross-
  validation in \ty{epos} by dropping the \ty{-x} option, which
  automatically sets the number of categories to 5. So the site
  frequency spectrum is divided into five parts of equal size. Four of
  these, that is 80\% of the data, are merged into the ``training''
  data set, from which the best configuration of levels is
  computed. This configuration is then applied to the remaining
  ``test'' data and the log-likelihood recorded. This is repeated in
  all five possible permutations and the final log-likelihood is the
  average printed as an intermediate result by \ty{epos}:
\begin{verbatim}
epos -s 13 -l 1000000 data/testF.dat 
#InputFile:	data/testF.dat
#Polymorphic sites surveyed:	16390
#Monomorphic sites surveyed:	983610
#m = 1; maximum Log(Likelihood): 1739015.209764	{2}
#m = 2; maximum Log(Likelihood): 1739074.268165	{2, 5}
#m = 3; no improvement
#Final Log(Likelihood):          12693399.625504
#Level	T[Level]	N[Level]
5	2.07e+05	2.39e+05
2	7.05e+05	1.66e+05
\end{verbatim}
The ``final'' likelihood is again computed for the full data set,
hence the large jump from  1739074.3 as the last intermediate
log-likelihood to  12693399.6 as the log-likelihood of the final
result. Notice that this is much smaller than the log-likelihood found
without cross validation, which was evidently the result of
over-fitting.
\item What happens if we use the exhaustive search with cross-validation?
\begin{verbatim}
epos -E 30 -s 13 -l 1000000 data/testF.dat 
\end{verbatim}
We get the same result as under the greedy strategy.
\item The division of the data during cross-validation is random. In
  order to fix the results for this tutorial, I set the seed for the
  random number generator using the \ty{-s} option. If we change the
  seed, we change the division of the data and with it the result:
\begin{verbatim}
epos -s 20 -l 1000000 data/testF.dat 
#InputFile:	data/testF.dat
#Polymorphic sites surveyed:	16390
#Monomorphic sites surveyed:	983610
#m = 1; maximum Log(Likelihood): 1737905.178788	{2}
#m = 2; maximum Log(Likelihood): 1737954.832488	{2, 27}
#m = 3; no improvement
