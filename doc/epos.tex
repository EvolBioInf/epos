...
\end{verbatim}
where \ty{-l 1e6} indicates that 1 Mb of sequence were surveyed. The result of this command is
\begin{verbatim}
...
upward as we get to the more distant past. For a sample of size $n$,
the expected time to its most recent common ancestor is proportional
to the population size \citep[p. 76]{wak09:coa}:
\[
E[T_{\rm MRCA}] - 4N\left(1-\frac{1}{n}\right).
\]
This means that as we go back in time only samples with large
population sizes are not completely coalesced yet, leading to an
upward drift in population size into the past. To counter this,
\ty{epos} only reports population sizes as long as more than one half
of the input samples have not reached their most recent common
ancestor. This fraction of uncoalesced samples can be changed using
the \ty{-f} option. Also, the variation in Figure~\ref{fig:con} is
